...
<div class="min-h-[80vh] flex items-center justify-center">
    <div class="w-full max-w-md p-8 bg-white rounded-3xl shadow-xl border border-slate-200 text-slate-900 relative overflow-hidden">
        <div class="text-center mb-8">
            ...
            <h2 class="text-2xl font-bold font-heading text-slate-900">Welcome back to CloudBox</h2>
            <p class="text-xs text-slate-500 mt-1">Sign in to access your files and cloud storage</p>
        </div>
        ...
        <form id="loginForm" method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <button type="submit" id="loginBtn" class="w-full py-3.5 px-4 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm transition shadow-md shadow-indigo-600/20">
                Sign In to Account
            </button>
        </form>
        ...
        <div class="mt-8 text-center text-xs text-slate-500 border-t border-slate-100 pt-6">
            ...
            <a href="{{ route('register') }}" class="text-indigo-600 hover:text-indigo-700 font-semibold ml-1">Create an account</a>
        </div>
    </div>
</div>
@endsection
